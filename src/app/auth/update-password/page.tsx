...
function UpdatePasswordForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(
          data.error || "Could not update your password. Please try again."
        );
        return;
      }
      setSubmitted(true);
  ...
  // No token in the URL at all — guide the member back to request one.
    ...
    <div className="rounded-2xl bg-white p-8 shadow-2xl">
      ...
      {submitted ? (
        ...
        <>
          ...
          {error && (
            <div className="mt-4 flex items-start gap-2 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 border border-red-200">
              <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
              <span>{error}</span>
            </div>
          )}
